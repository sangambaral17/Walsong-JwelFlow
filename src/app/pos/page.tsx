...
import { Scale, ShoppingCart, Printer, Plus, Trash2, QrCode, CheckCircle2, Usb, User, Search, X, Package } from "lucide-react";
...
export default function POSPage() {
    ...
    return (
        <div className="min-h-screen warm-bg-gradient text-foreground">
            ...
            {/* Live Rates Bar */}
            <div className="container mx-auto px-4 py-2 flex items-center justify-between no-print">
            ...
            <Dialog open={showInvoice} onOpenChange={setShowInvoice}>
                <DialogContent className="sm:max-w-2xl p-0 overflow-hidden bg-white text-black max-h-[90vh] overflow-y-auto">
                    ...
                    <DialogFooter className="p-4 pt-0 flex gap-2">
                        ...
                        {lastInvoice && lastInvoice.customer.phone && (
                            <Button
                                onClick={() => {
                                    const phone = lastInvoice.customer.phone.replace(/[^0-9]/g, "");
                                    const msg = `Namaste ${lastInvoice.customer.name}! 🙏\n\nThank you for shopping at *${profile.shop_name || "Walsong Jewellers"}*.\n\nInvoice #: ${lastInvoice.id}\nTotal: रू ${Number(lastInvoice.grandTotal).toLocaleString()}\nPaid: रू ${Number(lastInvoice.paidAmount).toLocaleString()}\nDue: रू ${Number(lastInvoice.balanceDue).toLocaleString()}\n\nWe appreciate your business! 📞 ${profile.phone || ""}`;
